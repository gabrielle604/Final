...
#### Scatterplot of the raw data to visually inpect it: dna_richness
...
#### Scatterplot of the raw data to visually inpect it: trad_richness
...
```{r m_0_fig, fig.align='center',fig.cap="Figure 6. Model 0: Species richness (logged) regressed on logged area in hectares."}
m_0 <- lm(species_richness_log ~ area_ha_log, data = sar_long) 

fig_m_0 <- ggplot(sar_long, aes(x = area_ha_log, y = species_richness_log)) + 
  geom_point() +
  geom_smooth() +
  labs(x = "Logged Area (hectares)", y = "(Logged) Species Richness") +
  theme_minimal() +
  stat_smooth(method = "lm", formula = lm(species_richness_log ~ area_ha_log, data = sar_long), se = FALSE)

fig_m_0
```

```{r m_2_fig, fig.align='center',fig.cap="Figure 7. Model 2: Species richness (logged) regressed on logged area in hectares by method. Peach is the eDNA metabarcoding method. Teal is the traditional methods."}
m_2 <- lm(species_richness_log ~ area_ha_log + method, data = sar_long) # Simple linear regression, logged species richness as a function of logged area and the method
...
fig_m_2 <- ggplot(sar_long, aes(x = area_ha_log, y = species_richness_log, color = method)) + 
  ...
  labs(x = "Logged Area (hectares)", y = "(Logged) Species Richness") +
  theme_minimal() +
  stat_smooth(method = "lm", formula = lm(species_richness_log ~ area_ha_log + method, data = sar_long), se = FALSE)
